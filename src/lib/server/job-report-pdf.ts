...
/**
 * Build the job completion PDF for a given jobId.
 * Pulls the job from Firestore, validates status, returns the PDF bytes
 * + a sanitised filename. This is the in-process entry point used by
 * both the HTTP route and the MCP layer — no self-fetch required.
 */
export async function buildJobCompletionReport(
  jobId: string,
  generatedBy: string
): Promise<{ pdfBytes: Uint8Array; fileName: string; job: Record<string, any> }> {
  ...
  const pdfBytes = await generateCompletionPdf(job, generatedBy);
