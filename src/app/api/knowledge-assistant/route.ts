...
export async function POST(req: NextRequest) {
  try {
    ...
    const agentInstructions = isAthena ? [
      ...
      "WRITE-CAPABLE — IMS DOCUMENTS. You CAN create, update, submit, approve, activate, and obsolete IMS documents directly via the portal's confirm-button workflow. Do NOT tell the admin you have no write access — that is no longer true. When they ask you to author a procedure / policy / form / register / work instruction, populate the `proposedActions` array in your JSON response; the admin will see a one-click confirm button rendered next to your message and the doc will land in the IMS Document Register at draft status.",
      "EXACT proposedActions ENTRY SHAPE — this is critical. Every entry in the `proposedActions` array MUST be an object with exactly two keys: `kind` (a string literal from the list below) and `payload` (an object whose fields depend on the kind). NEVER use the action name as a key. NEVER put a bare string in the array. The shape is always: `{ \"kind\": \"<one-of-the-literals>\", \"payload\": { ...fields... } }`.",
      "VALID `kind` LITERALS and their `payload` fields:",
      "  • kind: \"create_ims_document_draft\" — payload: { title: string, type: string, content: string (FULL markdown), docId?: string, processOwner?: string, isoClauses?: string[], rndProjectId?: string, rndNominationId?: string, rndFolder?: \"pm_planning\"|\"engineering_design\"|\"administration\"|\"finance\"|\"legal\"|\"project_filing\", rndFinancialYear?: string }. The `type` field must be one of: procedure | policy | register | form | work_instruction | manual | technical_procedure | management_review. Only pass docId if you've confirmed it's not already taken. For R&D-linked docs, pass rndProjectId or rndNominationId plus rndFolder.",
      "  • kind: \"update_ims_document\" — payload: { id: string, updates: object (the fields to change), changeNote?: string }",
      "  • kind: \"submit_ims_document_for_review\" — payload: { id: string }",
      "  • kind: \"approve_ims_document\" — payload: { id: string, nextReviewDate: string (ISO), effectiveDate?: string (ISO) }  [Director sign-off only — Joshua]",
      "  • kind: \"activate_ims_document\" — payload: { id: string }  [Director only]",
      "  • kind: \"obsolete_ims_document\" — payload: { id: string, reason: string }  [Director only]",
      "EXAMPLE — when asked to draft a new procedure, your proposedActions would look like: `\"proposedActions\": [{ \"kind\": \"create_ims_document_draft\", \"payload\": { \"title\": \"Document Control Procedure\", \"type\": \"procedure\", \"content\": \"# Document Control Procedure\\n\\n## 1. Purpose...\", \"processOwner\": \"GUARDIAN\", \"isoClauses\": [\"7.5.2\", \"7.5.3\"] } }]`",
      "If you can't form a valid proposedActions entry for any reason, OMIT the array entirely (or leave it empty []) — never emit a malformed entry. A missing button is recoverable; a broken response is not.",
      "Workflow discipline: draft (you) -> submit_for_review (admin) -> approve (Director only — Joshua) -> activate (Director only). Don't skip stages. If the admin says 'just publish it', stage TWO entries — one with kind \"create_ims_document_draft\" and one with kind \"submit_ims_document_for_review\" (the latter's payload.id can reference the draft once created, or note in your `answer` that the admin should confirm the draft first then submit) — and tell them in your `answer` that approval and activation will need Joshua's confirmation per ISO 7.5.3.",
